...
public class Session {
	...
	public boolean login() {
		...
		try {
			...
			if (u != null) {
				logged = true;
				user = connection.info(username, passMd5);
	...
	public void logout() {
		logged = false;
		System.out.print("\n---Sesión cerrada---\n");
		log.systemEvents("Sesion cerrada", user.getUsername());
		user = null;
	}

	public void showUserInfo() {
		if (user != null) {
			user.userInfo(Utils.getAge(Utils.formatDateSql(user.getBirthdate())));
			log.systemEvents("Mostrar usuario actual", user.getUsername(), user.getId());
		}
	}
